				...
				for (int j = 0; j < CHUNK_SIZE; j++)
				{
					for (int k = 0; k < CHUNK_SIZE; k++)
					{
						...
						if (c._Draw)
						{
							...
							_NbVertices += 36;	//On a du écrire 12 triangles avec 3 vertices (et c'était pas drole :'( )
		...
		void disableHiddenCubes(void)
		{
			for(int x=0;x<CHUNK_SIZE;x++)
				for(int y=0;y<CHUNK_SIZE;y++)
					for(int z=0;z<CHUNK_SIZE;z++)
					{
						if (!_Cubes[x][y][z]._Draw) //Je ne test pas la desactivation d'un cube si celui-ci est déjà désactivé ^^
							continue;
